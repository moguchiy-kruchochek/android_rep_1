...
class PostDaoImpl(private val db: SQLiteDatabase) : PostDao {
    companion object {
        ...
        CREATE TABLE ${PostColumns.TABLE} (
            ...
            ${PostColumns.COLUMN_PUBLISHED} LONG NOT NULL,
            ...
            ${PostColumns.COLUMN_IS_DELETED} BOOLEAN NOT NULL DEFAULT 0
        );
        """.trimIndent()
    }
    ...
    override fun save(post: Post): Post {
        val values = ContentValues().apply {
            ...
            put(PostColumns.COLUMN_PUBLISHED, System.currentTimeMillis())
        }
        val id = if (post.id != 0) {
            db.update(
                PostColumns.TABLE,
    ...
    override fun softDeleteById(id: Int) {
        db.execSQL(
            """
                UPDATE posts SET
                    isDeleted = CASE WHEN isDeleted THEN 0 ELSE 1 END
                WHERE id = ?;
            """.trimIndent(), arrayOf(id)
        )
    }
    ...
    private fun map(cursor: Cursor): Post {
        with(cursor) {
            return Post(
                ...
                isDeleted = getInt(getColumnIndexOrThrow(PostColumns.COLUMN_IS_DELETED)) != 0
            )
        }
    }
}
